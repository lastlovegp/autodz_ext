...
BEGIN
	...
	INSERT INTO qjsel SELECT 0,g.`code`,12,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
		...
		AND ((g2.`zr_close` < g2.`jx_30` AND g2.`zf` < -5 )
		       OR (g2.`zr_close` < g2.`jx_5` 
		            AND g2.`zr_close` < g2.`jx_10` 
		            AND g2.`zr_close` < g2.`jx_20` 
		            AND g2.`zr_close` < g2.`jx_30` 
		            AND g2.`zr_close` < g2.`jx_60`))
		AND g.`syl` <> 999
       ...
       SELECT DISTINCT CODE,price,jcdate,1 trantype,xhtype FROM (SELECT q.xhtype,q.code,q.price,IFNULL(q.jcdate,NOW()) jcdate 
         FROM qjsel q,gpzfrec g1 WHERE q.code=g1.code AND q.price < 50 AND ABS(TRUNCATE((g1.`jx_5`-g1.`zr_close`)/g1.`zr_close`*100,2)) < 20
