package com.base.common.report.excel;
...
import com.base.common.bean.impl.BaseVo;
...
public class ExcelFiller {
	...
	public ByteArrayOutputStream fill(HttpServletRequest request) {
		...
		try {
			...
			fillFields(wSheet);
			if (SysUtils.isNotEmpty(getExcelData().getFieldsList())) {
				// fillFields(wSheet);
			}
			wwb.write();
			wwb.close();
			...
			log.error(SysConstants.Exception_Head + "基于模板生成可写工作表出错了!");
			e.printStackTrace();
		}
		return bos;
	}
	...
	private void fillStatics(WritableSheet wSheet) {
		...
		for (int i = 0; i < statics.size(); i++) {
			...
			try {
				...
				log.error(SysConstants.Exception_Head + "写入静态对象发生错误!");
				e.printStackTrace();
			}
		}
	}
	...
	private void fillParameters(WritableSheet wSheet) {
		...
		for (int i = 0; i < parameters.size(); i++) {
			...
			try {
				if (type.equalsIgnoreCase(SysConstants.ExcelTPL_DataType_Number)) {
				...
				log.error(SysConstants.Exception_Head + "写入表格参数对象发生错误!");
				e.printStackTrace();
			}
		}
	}
	...
	private void fillFields(WritableSheet wSheet) throws Exception {
		...
		for (int j = 0; j < fieldList.size(); j++) {
			...
			if (object instanceof BasePo) {
				...
				log.error(SysConstants.Exception_Head + "不支持的数据类型!");
			}
			for (int i = 0; i < fields.size(); i++) {
				...
				try {
					...
					log.error(SysConstants.Exception_Head + "写入表格字段对象发生错误!");
					e.printStackTrace();
				}
			}
		}
		...
		for (int i = 0; i < variables.size(); i++) {
			...
			try {
				if (type.equalsIgnoreCase(SysConstants.ExcelTPL_DataType_Number)) {
					...
					if (SysUtils.isEmpty(content) && !key.equalsIgnoreCase("nbsp")) {
						content = key;
					}
				...
				log.error(SysConstants.Exception_Head + "写入表格变量对象发生错误!");
				e.printStackTrace();
			}
		}
	}
	...
	private static String getType(String pType) {
		String type = SysConstants.ExcelTPL_DataType_Label;
		if (pType.indexOf(":n") != -1 || pType.indexOf(":N") != -1) {
			type = SysConstants.ExcelTPL_DataType_Number;
		}
		return type;
	}
