...
namespace MdTZ
{
    class StaUtil
    {
        ...
        {
            return code.Replace("sh", "").Replace("sz", "").Replace("SZ", "").Replace("SH", "");
        }
        ...
        /// <returns></returns>
        public static bool getShangSQs(BarTotal total)
        {
            bool rtnFalg = false;
            if (total.gpTotal.jj10 < total.gpTotal.jj5 && 
                Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2)
                       ...
                       > 0 && Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2) < total.gpTotal.jj5 * 0.02
                        && Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2) > total.gpTotal.jj5 * 0.002)
            {
                rtnFalg = true;
            }
        ...
        public static bool getShangSQs(TranBean total)
        {
            bool rtnFalg = false;
            if (total.gpTotal.jj10 < total.gpTotal.jj5 &&
                Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2)
                       - Math.Round(total.gpTotal.preGpTotal.jj5 - total.gpTotal.preGpTotal.jj10, 2) > 0 
                        && Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2) < total.gpTotal.jj5 * 0.02
                        && Math.Round(total.gpTotal.jj5 - total.gpTotal.jj10, 2) > total.gpTotal.jj5 * 0.002
                        && total.gp_time_qs == 1)
            {
                total.xh_type = GPConstants.XH_BUY_GKJJ;
                rtnFalg = true;
            }
        ...
        public static bool getXiadGoodQs(Tick tick, TranBean total)
        {
            bool rtnFalg = false;
            if (total.lastDailyBar.high < total.gpTotal.jj10
                    && tick.last_price < total.gpTotal.jj10
                    && total.gpTotal.jj10 > total.gpTotal.jj5
                    && Math.Round(total.gpTotal.jj10 - total.gpTotal.jj5, 2) < total.gpTotal.jj10 * 0.02
                    && Math.Round(total.gpTotal.jj10 - total.gpTotal.jj5, 2) > total.gpTotal.jj10 * 0.002
                    && total.gp_time_qs == 1
                )
            {
                total.xh_type = GPConstants.XH_BUY_SSZJ;
                rtnFalg = true;
            }
        ...
        /// <param name="total"></param>
        /// <returns></returns>
        public static bool getXiadGoodQs(Bar bar, BarTotal total)
        {
            bool rtnFalg = false;
            if (total.lastDailyBar.high < total.gpTotal.jj10
                    ...
                    && total.gpTotal.jj10 > total.gpTotal.jj5
                    && Math.Round(total.gpTotal.jj10 - total.gpTotal.jj5, 2) < total.gpTotal.jj10 * 0.02
                    && Math.Round(total.gpTotal.jj10 - total.gpTotal.jj5, 2) > total.gpTotal.jj10 * 0.009
                )
            {
                rtnFalg = true;
            }
