...
namespace MdTZ
{
    public class SinaAPI
    {
        ...
        {
            string rtnCode = code;
            if (!string.IsNullOrEmpty(code) && code.IndexOf("sh") == -1 && code.IndexOf("sz") == -1)
            {
                if (code.IndexOf("6") == 0)
                {
                    rtnCode = "sh" + code;
    ...
    * */
        ...
        {                     
            if (String.IsNullOrEmpty(code))
            {
                code = "sh000001,sh000016,sz399001,sz399006";
            }
            else
            {                            
                code += "sh000001,sh000016,sz399001,sz399006";
        ...
        private static List<GuoPiao> getSubGPList(string[] fieldList, string codes)
        {
            ...
            for (int i = 0; i < fieldList.Length; i++)
            {
                ...
                if (gp.code.Length < 6)
                {
                    gp.kpj = Convert.ToDouble(fields[2]); //开盘价
                    gp.jrzgj = Convert.ToDouble(fields[3]);//最高价
                    gp.jrzdj = Convert.ToDouble(fields[4]);//最低价
                    gp.zrspj = Convert.ToDouble(fields[5]); //昨日收盘价  
                    gp.b_now = Convert.ToDouble(fields[6]);//买价，即“买一”报价
                    gp.s1_num = Convert.ToDouble(fields[7]);//卖价，即“卖一”报价
                    gp.dqj = Convert.ToDouble(fields[8]); //最新价                                                                         
                    gp.date = fields[17];//日期
                    gp.time = System.DateTime.Now.ToShortTimeString();
                ...
                if (gp.jrzdj > 0)
                {
                    gp.zhengf = Math.Round(((gp.jrzgj - gp.jrzdj) / gp.jrzdj) * 100, 2);
                }              

                //分时均价
                if (gp.code.Equals("sz399006") 
                    || gp.code.Equals("sz399001")
                    || gp.code.Equals("sh000016") 
                    || gp.code.Equals("sh000001")
                )
                {                   
                    gp.jj = Math.Round((gp.jrzdj + gp.jrzgj) / 2, 2);
                    if (gp.code.Equals("sz399006"))
                    {
                        gp.jj = Math.Round((gp.jj + gp.jrzgj) / 2, 2);
                    }
                }
                else if (gp.cj_num > 0)
                {
       ...
       * */
        ...
        public static DataTable getGuoPiaoHisDataFromYaoh(DateTime bdate, DateTime edate, string code)
        {
            ...
            string url = "http://table.finance.yahoo.com/table.csv?" + "a=" + (bdate.Month - 1).ToString()
                ...
                + "&f=" + edate.Year.ToString() + "&s=" + yh_codes;
        ...
        **/    
        public static void webGetCwCallBack(WebBrowser gp_web, List<string> codeList, List<string> loadCodeList)
        {
            ...
            if (codeList.Count > 0)
            {
                code = codeList[0].Replace("sh", "").Replace("sz", "").Replace("SH", "").Replace("SZ", "");
            }
