       ...
       * */
        ...
        * */
        private static string getGpsForLoadYaohHis()
        {
            MySqlHelper helper = new MySqlHelper();
            MySqlParameter[] parms = new MySqlParameter[] { };
            DataTable dataTable = helper.ExecuteDataTable("SELECT code FROM gp", parms);
        ...
        public static string getHeyCodesFromDb()
        {

            DataTable dataTable = GPUtil.helper.ExecuteDataTable("SELECT DISTINCT code FROM hey order by code", GPUtil.parms);
            StringBuilder codes = new StringBuilder();
            if (dataTable != null)
            {
                foreach (DataRow row in dataTable.Rows)
        ...
        public static string getSinaHeyCodesFromDb()
        {

            DataTable dataTable = GPUtil.helper.ExecuteDataTable("SELECT DISTINCT slcode FROM hey WHERE code NOT IN ('SHSE.000016')", GPUtil.parms);
            StringBuilder codes = new StringBuilder();
            if (dataTable != null)
            {
                foreach (DataRow row in dataTable.Rows)
                {
                    codes.Append(row["slcode"]).Append(",");
                }
            }
            //Console.Write("codes:" + codes.ToString());
            return codes.ToString();
        ...
        public static double getDjsTodayZf()
        {

            DataTable dataTable = GPUtil.helper.ExecuteDataTable("SELECT zf FROM gptoday where code='int_dji'", GPUtil.parms);
            double zf = 0;
            if (dataTable != null)
            {
                foreach (DataRow row in dataTable.Rows)
                {
                    zf = Convert.ToDouble(row["zf"]);
                }
            }
            //Console.Write("codes:" + codes.ToString());
            return zf;
        ...
        {
            String sql = "SELECT CODE FROM gp WHERE CODE NOT IN " +
                "(SELECT DISTINCT CODE FROM gpcw where date >='" + DateTime.Now.Year.ToString() + "-01-01')";

            //String sql = "SELECT CODE FROM gpcw";
            DataTable dataTable = GPUtil.helper.ExecuteDataTable(sql, GPUtil.parms);

            if (dataTable.Rows.Count > 0)
            {
                 sql = "SELECT CODE FROM gp WHERE CODE NOT IN "+
                         "(SELECT DISTINCT CODE FROM gpcw where date >'" + DateTime.Now.Year.ToString() + "-01-01')";
                 dataTable = GPUtil.helper.ExecuteDataTable(sql, GPUtil.parms);
            }
            else
            {
                sql = "SELECT CODE FROM gp";
                dataTable = GPUtil.helper.ExecuteDataTable(sql, GPUtil.parms);
            }
        ...
        public static void loadAllHisDataFromYaohu(DateTime bdate, DateTime edate)
        {
            ...
            loadBatchHisDataFromYaohu(gpcodes, bdate, edate);
            

        }

        public static void loadBatchHisDataFromYaohu(string codes,DateTime bdate, DateTime edate)
        {
            string gpcodes = codes;
         ...
         * 从sina 获取当天数据
         * */
        public static void saveTodayHisDataFromSina(bool loadDpFlag, long tick_cnt)
        {           
            string codes = GPUtil.codes;           
            ...
            {
                GPUtil.helper.ExecuteNonQuery("TRUNCATE gptoday", GPUtil.parms);
            }

            if (loadDpFlag)
            {
                List<string> dqsqlList = new List<string>();
                if (dpList != null && dpList.Count > 0)
                {
                    foreach (DaoPan dp in dpList)
                    {
                        sql.Clear();
                        sql.Append("insert into gptoday(code,dqj,cj_num,zf,date) values ('")
                            .Append(dp.code).Append("',")
                            .Append(dp.zs).Append(",")
                            .Append(dp.cjl).Append(",")
                            .Append(dp.zdl).Append(",'")
                            .Append(GPUtil.nowTranDate).Append("')");

                        dqsqlList.Add(sql.ToString());
                    }

                    if (dqsqlList.Count > 0)
                    {
                        MysqlHelper1.ExecuteNoQueryTran(dqsqlList);
                    }
            ...
            foreach (GuoPiao gp in gps)
            {
                ...
                sql.Append("insert into gptoday(code,kpj,dqj,zrspj,jrzgj,jrzdj,cj_num,cj_amt,zf,date,zhengf,jj,tr,trantime) values ('")
                .Append(gp.code).Append("',")
                .Append(gp.kpj).Append(",")
                .Append(gp.dqj).Append(",")
                ...
                .Append(gp.date).Append("',")
                .Append(gp.zhengf)
                .Append(",").Append(gp.jj).Append(",").Append(gp.tr).Append(",'").Append(gp.time)
                .Append("');");
        ...
        public static void saveTodayTickDataFromGm(String bTime,String eTime, long tick_cnt)
        {         
            if (MdComm.md == null)
            {
                MdComm.initMd();
            ...
            List<Tick> tickList = MdComm.md.GetTicks(symbols, bTime, eTime);
            ...
            foreach (Tick tick in tickList)
            {
                ...
                if (tick.pre_close == 0 || tick.low == 0 || tick.cum_volume == 0)
                {
                    continue;
                }
                ...
                .Append(tick.open)
                .Append(",")
                .Append(tick.last_price)
                .Append(",")
                .Append(tick.pre_close)
                .Append(",")
                .Append(tick.high)
                .Append(",")
                .Append(tick.low)
                .Append(",")
                .Append(tick.cum_volume)
                .Append(",")
                .Append(Math.Round(tick.cum_amount / 100000000,2))
                .Append(",")
                .Append(Math.Round(((tick.last_price - tick.pre_close) / tick.pre_close) * 100, 2))
                .Append(",'")
                .Append(Convert.ToDateTime(tick.strtime).ToString())
                .Append("',")
                .Append(Math.Round(((tick.high - tick.low) / tick.low) * 100, 2))
                .Append(",")
                .Append(Math.Round(tick.cum_amount/tick.cum_volume,2))
                .Append(",")
                .Append("0,'")
                .Append(Convert.ToDateTime(tick.strtime).ToString("HH:MM:ss"))
                .Append("');");
           ...
           // GPUtil.helper.ExecuteProcNoOut("call qj_process();"); 
