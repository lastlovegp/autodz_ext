...
namespace MdTZ
{
    ...
    class HGStaUtil
    {
        ...
        //大盘编号
        public const string DP_CODES = "sh000001,sz399001,sz399006";
               ...
               TranBean tick)
        {
            ...
            for (int i = barList.Count - 1; i >= 0; i--)
            {
               ...
               //非大盘时候
                ...
                //记录跌停到现在天数
                if (total.dt_bar_num == 0 && zf < -3.1)
                {
                    total.dt_bar_num = cnt;
                }

                //记录跌停到现在天数
                if (total.dt_max_bar_num == 0 && zf < -8)
                {
        ...
        public static TranBean getTranBean(Strategy strategy,Dictionary<string, TranBean> tranStas, Tick tick, StrateSta sta)
        {
            ...
            double tr = getTr(tick);
            //振幅
            double zhengf = 0;
            if (tick.low > 0)
            {
                zhengf = Math.Round(((tick.high - tick.low) / tick.low) * 100, 2);
            }
            ...
            if (tranStas.ContainsKey(tick.sec_id))
            {
                ...
                if (bean.strtime.Substring(0, 10).Equals(tick.strtime.Substring(0, 10)))
                {
                    ...
                    //if (bean.high == 0)
                    //{
                    //    bean.high = Math.Round(tick.high, 2);
                    //}

                    //if (bean.low == 0)
                    //{
                    //    bean.low = Math.Round(tick.low, 2);
                    //}                 

                    //if (bean.btime >= 9.30)
                    //{
                    //    if (String.IsNullOrEmpty(bean.lowest_time))
                    //    {
                    //        bean.lowest_time = bean.tick.strtime;
                    //    }

                    //    if (String.IsNullOrEmpty(bean.highest_time))
                    //    {
                    //        bean.highest_time = bean.tick.strtime;
                    //    }        

                    //    bean.high = Math.Round(bean.high < tick.high ? tick.high : bean.high, 2);
                    //    bean.low = Math.Round(bean.low > tick.low ? tick.low : bean.low, 2);                                                                     

                    //    if (bean.btime >= 9.30 && bean.btime <= 10.30)
                    //    {
                    //        bean.low1 = bean.low;
                    //        bean.high1 = bean.high;
                    //    }
                    //    else if (bean.btime > 10.30 && bean.btime <= 11.30)
                    //    {                           
                    //        bean.low2 = bean.low;
                    //        bean.high2 = bean.high;
                    //    }
                    //    else
                    //    {
                    //        bean.low3 = bean.low;
                    //        bean.high3 = bean.high;
                    //    }

                    //    bean.highest_time = bean.high < tick.last_price ? tick.strtime : bean.highest_time;
                    //    bean.lowest_time = bean.low > tick.last_price ? tick.strtime : bean.lowest_time;  
                    //}

                    ////1分钟涨幅
                    //if (bean.tick != null && bean.seq > 0 && bean.seq % 60 == 0)
                    //{
                    //    if (bean.pre_tick_price > 0)
                    //    {
                    //        bean.zf1sec = Math.Round((bean.tick.last_price - bean.pre_tick_price) / bean.pre_tick_price, 5);
                    //    }
                    ...
                    //    {
                    //        bean.tp_cnt++;
                    //        if (bean.last_tp_tick_seq > 0)
                    //        {
                    //            bean.tp_tick_seq_cg_sum += (bean.seq - bean.last_tp_tick_seq) / 60;
                    //        }
                    //        bean.last_tp_tick_seq = bean.seq;
                    //        bean.pre_tp_last_price = bean.tp_last_price;
                    //        bean.tp_last_price = Math.Round(bean.tick.last_price, 3);
                    //        bean.tp_seq = bean.seq;
                    //    }

                    //    bean.pre_pre_tick_price = bean.pre_tick_price;
                    //    bean.pre_tick_price = Math.Round(bean.tick.last_price, 2);
                    //}

                    //if (bean.pre_tick_price > 0
                    //    && bean.timejj > 0
                    //    && bean.tick.last_price > 0
                    //    && bean.pre_tick_price > bean.timejj
                    //    && bean.tick.last_price < Math.Round(bean.timejj, 2))
                    //{
                    //    bean.tp_cnt = 0;
                    //    bean.tp_tick_seq_cg_sum = 0;
                    //    bean.last_tp_tick_seq = 0;
                    //    bean.pre_tp_last_price = 0;
                    //    bean.tp_last_price = 0;
                    //    bean.tp_seq = 0;

                    //    bean.is_low_timejj = true;
                    //}
                    //else if (bean.pre_tick_price > 0
                    //    && bean.timejj > 0
                    //    && bean.tick.last_price > 0
                    //    && bean.pre_tick_price < Math.Round(bean.timejj, 2)
                    //    && bean.tick.last_price > bean.timejj)
                    //{
                    //    bean.is_low_timejj = false;
                    //    bean.tp_timejj_num++;
                    //}
                    #endregion
                }
                else
                {
                    ...
                    //重新加载统计信息
                    loadGpTotalList(strategy, MdComm.getJJCode(tick.sec_id), bean);
                    ...
                    //昨日bar
                    List<DailyBar> bars = strategy.GetLastNDailyBars(MdComm.getJJCode(bean.code),
                                4, t_tran_time.ToString("yyyy-MM-dd HH:MM:ss"));
                    ...
                    {
                        GPUtil.helper.ExecuteNonQuery("UPDATE POSITION SET sumzf=(" + bean.tick.last_price + "-position.vwap)/position.vwap*100 WHERE available > 0 and code='" + bean.code + "'");
                    }
            ...
            else
            {
                ...
                if (bars != null && bars.Count >= 3)
                {
                    ...
                    //检查有无除权
                    foreach (DailyBar mBar in bars)
                    {
                        if (preBar != null && Math.Round((preBar.close-mBar.close)/preBar.close,2)*100 < -10.5)
                        {
                            bean.cqFlag = 1;
                            break;
                        }
                ...
                if (bean.isDp)
                {
                    sta.positionDbList = GPUtil.getAllPositionDb();
                }             
                ...
                //本地库
                if (bean.gpInfo != null && bean.gpInfo.zbflag == 0)
                {
                    bean.positionDb = GPUtil.getPositionDb(bean.code);
                }    
            }
            ...
            if (bean.btime > 9.31)
            {
                if (tick.last_price > bean.timejj)
                {
                    bean.more_timejj_cnt++;
                }
                else if (tick.last_price < bean.timejj)
                {
                    bean.less_timejj_cnt++;
                }

                if (tick.last_price >= Math.Round(bean.timejj * (1 - 0.005), 2))
                {
                    bean.more_timejj_cnt_ext++;
                }
                else
                {
                    bean.less_timejj_cnt_ext++;
                }               
            }
            ...
            if (bean.isDp)
            {
                if (bean.seq % (MdComm.str_mode == 2 ? 5 : 100) == 0) {
               ...
               //插入数据库日志
               if ((MdComm.str_mode == 2 || MdComm.str_mode == 3) && bean.seq % 60 == 0)
               {
        ...
        public static BarTotal getBarTotal(Strategy strat,Dictionary<string, BarTotal> barDic, Bar bar, StrateSta sta)
        {
            ...
            if (barDic.ContainsKey(bar.sec_id))
            {
                ...
                if (bean.btime > 9.30)
                {
                    bean.highest_time = bean.high > bar.high ? bean.highest_time : bar.strtime;
                    bean.lowest_time = bean.low < bar.low ? bean.lowest_time : bar.strtime;

                    bean.high = Math.Round(bean.high > bar.high ? bean.high : bar.high, 2);
                    bean.low = Math.Round(bean.low < bar.low ? bean.low : bar.low, 2);
                    bean.volume = bean.volume > bar.volume ? bean.volume : bar.volume;
                }
                ...
                if (bean.strtime.Substring(0,10).Equals(bar.strtime.Substring(0,10))) {
                     ...
                     //重新获取
                     List<DailyBar> bars = strat.GetLastNDailyBars(MdComm.getJJCode(bean.code),
                     ...
                     //重新加载统计信息
                     loadGpTotalList(strat, MdComm.getJJCode(bar.sec_id), bar.strtime, bar.strtime);
                    ...
                    && (bean.seq == 1 || bean.seq % 20 == 0))
            {
                ...
                //插入数据库日志
                if (MdComm.str_mode == 2 && bean.seq % 10 == 0)
                {
