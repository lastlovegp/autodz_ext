    ...
    class Strategy_JCMR : Strategy
    {
        ...
        public bool isNeedOpenOrder(StrateSta sta, TranBean bean, int tran_mode)
        {
            ...
            if (
                ...
                && (bean.zf > 1 && sta.dp.zf <= -0.35 ? false: true)
                && bean.btime > (bean.zf > 1 ? 9.35 : 14.45)
                && bean.btime <= 14.57
               )
            {
        ...
        public override void OnTick(Tick tick)
        {
            ...
            if (MdComm.str_mode == 6 && t_time.Hour == 9 && t_time.Minute < 30)
            {
                return;
            }
            ...
            try
            {
                ...
                //大盘指数的时候过滤掉
                if (MdComm.str_mode == 6 && (bean == null || bean.btime < 9.30 || bean.seq < 20 || bean.btime > 15.02))
                {
                    return;
                }
                ...
                && bean.sell_cnt == 0)
            {
                ...
                double zy = 4;
                double zs = -8;
                if (bean.gpTotal.rsi24 <= 45 || bean.gpTotal.sum30Zf < 8)
                {
                    zy = 6;
                }
