...
BEGIN
      ...
      IF (INSTR(dp_zf_str,'3') > INSTR(dp_zf_str,'5')) THEN
        SET is_dp_qs = -1;
      END IF;          
      IF (is_cyb_qs+is_dp_qs+is_zxb_qs < 0 AND zxp_zf+dp_zf+cyb_zf < 1.2) THEN                          
         /*关闭*/                            
         UPDATE `gpparam` SET tranflag = '0';                  
      /*UELSE
	 PDATE `gpparam` SET tranflag = '1';*/
      END IF;
	...
	/*记录大盘最低*/
      ...
      DELETE FROM POSITION  WHERE `sumzf` <= -0.02 AND DATE(`indate`) = DATE(NOW()) AND `available_today` > 0 ;
	...
	INSERT INTO qjsel SELECT 0,g.`code`,12,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0 
       ...
       INSERT INTO qjsel SELECT 0,g.`code`,13,g3.dqj,g3.zf,CONCAT(DATE_FORMAT(g3.`date`,'%Y-%m-%d'),' ',g3.`trantime`),0 
		 FROM todaytc g1,gp g,gpzfrec g2,gptoday g3 WHERE g1.`code`=g.`code` AND g.`code`=g2.`code` AND g2.`code`=g3.`code`
		 AND xh_13_cnt < 3		
		 AND g2.`rsi_6` < 60  
		 AND g2.`rsi_12` < 60 
		 AND g2.`rsi_24` < 60	
		 ...
		 AND g.syl < 60	
		 AND g3.dqj < 60					 			
		 ORDER BY g1.sx,g.ltsz,g.syl,g.zsz LIMIT 0,1;                 	 		         
