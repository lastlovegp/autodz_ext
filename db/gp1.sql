...
) ENGINE=InnoDB AUTO_INCREMENT=95050 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `gpyaohhis` (
  ...
  `zf` decimal(8,2) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=90360 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `add_gpsel`(sel_type VARCHAR(10))
BEGIN                 
	DECLARE limit_cnt INTEGER DEFAULT 250;  
	
	DECLARE gpcodes text;                      
        
        DELETE FROM gpsel_bak;   
                            
        IF (sel_type ='yzg') THEN
          INSERT INTO gpsel_bak SELECT DISTINCT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `优质股` rl,gp g,gpzfrec r,gptoday g3
		WHERE rl.code=g.code AND g.`code`=r.`code` and g.code=g3.code and g3.cj_amt > 0	
		and g.ltsz < 200 and g.syl< 50					
		ORDER BY g3.dqj,g.ltsz LIMIT 0,limit_cnt;     	 				   	     
	ELSEIF (sel_type ='all_cyb') THEN          
	   INSERT INTO gpsel_bak SELECT DISTINCT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `多头走势` g WHERE g.code LIKE '3%'
		 AND g.code NOT IN (SELECT CODE FROM xsjj)
		 AND g.code NOT IN (SELECT gpcode from `黑天鹅`) LIMIT 0,limit_cnt;	
        ELSEIF (sel_type ='zxg') THEN           
           /*放入股票池*/
           SELECT GROUP_CONCAT(DISTINCT t.`code`) INTO gpcodes  FROM (SELECT CODE FROM gpsel g WHERE g.flag >= 0) t;		
           UPDATE `stratey` SET gpcodes = gpcodes; 
                                      	 	   	  	     
	ELSEIF (sel_type ='hengpan_dt') THEN          
	   INSERT INTO gpsel_bak SELECT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `多头走势` g WHERE 	        
		  g.code NOT IN (SELECT CODE FROM xsjj) 
		  AND g.code NOT IN (SELECT gpcode FROM `黑天鹅`) 
		  LIMIT 0,limit_cnt;
        ELSEIF (sel_type ='lhg') THEN          	   
	   INSERT INTO gpsel_bak SELECT DISTINCT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `优质股` rl,gp g,gpzfrec r,gptoday g3
		WHERE rl.code=g.code AND g.`code`=r.`code` AND g.code=g3.code AND g3.cj_amt > 0	
		AND g.ltsz < 200 AND g.ltsz=g.`zsz` AND g.syl< 50	
		AND g.code NOT IN (SELECT CODE FROM xsjj) 
		AND g.code NOT IN (SELECT gpcode FROM `黑天鹅`) 				
		ORDER BY g3.dqj,g.ltsz        		
		  LIMIT 0,limit_cnt;
        ELSEIF (sel_type ='ydgc') THEN          	   
	   INSERT INTO gpsel_bak SELECT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `异动观察` g WHERE 	        
		  g.code NOT IN (SELECT CODE FROM xsjj) 
		  AND g.code NOT IN (SELECT gpcode FROM `黑天鹅`) 
		  LIMIT 0,limit_cnt;
        ELSEIF (sel_type ='zrdz') THEN          	   
	   INSERT INTO gpsel_bak SELECT DISTINCT 0 id,2,g.code,g.name,4 flag,1,NOW() FROM `涨跌幅排行` g,gpzfrec g1 WHERE 
	           g.code = g1.code AND g.zf > 1 AND g1.`pj_15_zhengfu` > 3       
		  AND g.code NOT IN (SELECT CODE FROM xsjj) 
		  AND g.code NOT IN (SELECT gpcode FROM `黑天鹅`) ORDER BY g.zf DESC	 
		  LIMIT 0,limit_cnt;
        ELSEIF (sel_type ='lhb') THEN          	   
	    INSERT INTO gpsel_bak SELECT 0,2,g.`code`,g1.`name`,4,1,NOW() FROM gptoday g,gp g1 WHERE g.`code`=g1.`code` AND  g.`zf` > 9.1;
        ELSEIF (sel_type ='hyys') THEN          
	     INSERT INTO gpsel_bak SELECT DISTINCT 0 id,g.code,g.name,4 flag,1,NOW() FROM gp g,hyzfrec h,gpzfrec g2 ,gptoday g3 
	      WHERE g.`code`=g2.`code` AND g2.`code`=g3.`code`
	      AND g3.`dqj` > g2.`top_30` LIMIT 0,limit_cnt; 
	ELSEIF (sel_type ='gnys') THEN          
	     INSERT INTO gpsel_bak SELECT distinct 0 id,2,g.code,g.name,4 flag,1,NOW() FROM gp g,gn2gp gn,gnzfrec gn1,gptoday g2,gpzfrec g3
	         WHERE g.`code`=gn.`gpcode` 
	         AND g.`code`=g2.`code`
	         and g.code=g3.code
		 AND gn.`gn`=gn1.`gn` 
		 AND g2.`dqj` > g3.`top_30`  LIMIT 0,limit_cnt; 
	ELSEIF (sel_type ='dbd') THEN          
	     INSERT INTO gpsel_bak SELECT DISTINCT 0 id,2,g1.code,g1.`name`,4 flag,1,NOW() 
	        FROM `gpzfrec` g,gp g1,gptoday g2 
	        WHERE g.`code`=g1.`code` 
	        AND g1.`code`=g2.`code` 
	        and g2.cj_amt > 0	       	  		
		AND g.`pj_15_zhengfu` > 3 
		AND g2.`dqj` > g.`top_30`  lIMIT 0,limit_cnt;  		    
        end if;    
        
       IF (sel_type <> 'zxg') THEN           
		
         /*放入股票池*/
         SELECT GROUP_CONCAT(DISTINCT g.`code`) INTO gpcodes  FROM gpsel_bak g,gptoday g2,gp g3 WHERE 
          g.code=g3.code AND g.code=g2.code AND g2.cj_amt >0 ORDER BY g3.ltsz;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `buy_sell`(dc_flag INT)
BEGIN
	/**买人 */
       SELECT DISTINCT CODE,300 buynumber,price,jcdate,trantype,xhtype,TYPE FROM (
	...
	FROM POSITION p,gptoday g,gp g1 
		WHERE p.`code`=g.`code` AND g.code=g1.code
		 ...
		 AND g.zf < 0
		 AND p.`sumzf` < IF(p.`inxh` IN (1,2,3) AND dc_flag=1,-1,-2)					 	 
	...
	UNION ALL
		...
		WHERE p.`code`=g.`code` 
		 AND p.`available` > 0
		 AND (p.available - p.`available_today`) > 0		 
		 AND p.`sumzf` > IF(p.`inxh` in (1,2,3) AND dc_flag=1,0,1)	
		 AND g.zf < 9		
		 AND DATE(p.`indate`) < DATE(NOW())
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_bc`(sz_50_zf DOUBLE,cyb_zf DOUBLE,dp_zf DOUBLE,dc_flag INT)
BEGIN
                             
      /*补仓*/ 
      INSERT INTO qjsel SELECT DISTINCT 0,g.`code`,5,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0
                FROM position p,gp g,gptoday g1,gpsel g2 WHERE p.`code`=g.`code` AND g.`code`=g1.`code` AND g.`code`=g2.`code`                 
                and p.`available` > 0  
                and p.`available_today` = 0 
                and g1.zf < 0 
                and g1.dqj > g1.`jrzdj`
		and g1.`cj_amt` > 0
		and dc_flag = if (g2.`gn` = 'sz50',1,0)
                AND p.`sumzf` > -2 and p.`sumzf` < -4 
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_cdqsg`(in_xh INTEGER)
BEGIN
	...
	INSERT INTO qjsel SELECT 0,g.`code`,in_xh,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0 
		...
		AND g1.dqj < g2.`jx_10`
		AND g1.dqj < g2.`jx_30`			 											
		AND TIME_TO_SEC(g1.`trantime`) > TIME_TO_SEC("10:40:00")
		ORDER BY gs.`gpflag` DESC,(g2.`rsi_6`+g2.`rsi_12`+g2.`rsi_24`) LIMIT 0,1;  		
...
/* Procedure structure for procedure `cl_dc` */

/*!50003 DROP PROCEDURE IF EXISTS  `cl_dc` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_dc`(sz_50_zf DOUBLE,cyb_zf DOUBLE,dp_zf DOUBLE)
BEGIN
      ...
      INSERT INTO qjsel SELECT DISTINCT 0,g.`code`,4,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0
                ...
                and gs.`gn` = 'sz50'  and gs.`gpflag` = 1                                       
		AND g1.kpj > 0 		
		AND g1.cj_amt > 0																	
		AND g1.zf > 0.5 
		and g1.zf < 1.5		
		AND sz_50_zf > 0.1 AND cyb_zf < -0.18
		AND sz_50_zf > cyb_zf
		AND TIME_TO_SEC(g1.`trantime`) < TIME_TO_SEC("14:52:00")
		LIMIT 0,2;         
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_getgnztgps`(in_xh INTEGER,sz_50_zf DOUBLE,cyb_zf DOUBLE,dp_zf DOUBLE,dc_flag INT)
BEGIN
	 ...
	 INSERT INTO qjsel SELECT 0,g2.`code`,in_xh,g2.dqj,g2.zf,CONCAT(DATE_FORMAT(g2.`date`,'%Y-%m-%d'),' ',g2.`trantime`),0 
		...
		WHERE g.`code`=g1.`gpcode` AND g.`code`=g2.`code` AND g.`code`=g3.`code`
		  ...
		  AND g3.zf < 3
		  AND g3.`zf3` < 5
		  AND g3.`rsi_6` < 80 AND g3.`rsi_12` < 80
		  AND ((sz_50_zf < -0.1 AND cyb_zf > 0.18) || dp_zf > 0.15)
		  AND sz_50_zf < cyb_zf
		  AND TIME_TO_SEC(g2.`trantime`) < TIME_TO_SEC("14:52:00") 	  							 			      		 
		  ORDER BY (g3.`rsi_6`+g3.`rsi_12`+g3.`rsi_24`) LIMIT 0,1;			
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_gethyztgps`(in_xh INTEGER,sz_50_zf DOUBLE,cyb_zf DOUBLE,dp_zf DOUBLE,dc_flag INT)
BEGIN
...
BEGIN
	 ...
	 AND g2.`cj_amt` > 0 	  
	 AND g2.`dqj` < 60 		
	 AND g1.zf < 3 and g1.zf > -2  			
	 AND g.code NOT IN (SELECT CODE FROM `xsjj$`)	
	 AND g.code NOT IN (SELECT p.code FROM `position` p WHERE p.`available_today` > 0) 	
	 AND g.ltsz < 100
	 AND g.syl < 60	 		 	  	
	 ORDER BY (g1.`rsi_6`+g1.`rsi_12`+g1.`rsi_24`) LIMIT 0,1; 
	...
	/**合约套利*/
	 ...
	 FROM `合约排名股票1` g,gpzfrec g2,gptoday g1,gp g3 WHERE g.`code`=g2.`code` AND g2.`code`=g1.`code` AND g1.`code`=g3.`code`	 
      ...
      INSERT INTO qjsel SELECT 0,g2.`code`,in_xh,g4.dqj,g4.zf,CONCAT(DATE_FORMAT(g4.`date`,'%Y-%m-%d'),' ',g4.`trantime`),'0'
		...
		AND g3.`zf` < 3
		AND g3.`zf2` < 10				
		ORDER BY (g3.`rsi_6`+g3.`rsi_12`+g3.`rsi_24`);	     
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_zxp`(in_xh INTEGER,sz_50_zf DOUBLE,cyb_zf DOUBLE,dp_zf DOUBLE,dc_flag INT)
BEGIN
      ...
      INSERT INTO qjsel SELECT DISTINCT 0,g.`code`,in_xh,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0
                FROM gpsel gs,gp g,gptoday g1,gpzfrec g2 WHERE gs.`code`=g.`code` AND g.`code`=g1.`code` AND g.`code`=g2.`code`  
                AND in_cnt < 1                
                and gs.`gn` <> 'zs'  and gs.`gpflag` in (1,2)                                     
		AND g1.kpj > 0 		
		AND g1.cj_amt > 0
		and cyb_zr_zf < 1.2
		and dc_flag = 0			
    ...
    /*放入股票池*/
     if (in_mode =4) then
	     ...
	     SELECT GROUP_CONCAT(DISTINCT t.`code`) INTO gpcodes  
	      FROM (SELECT CODE FROM gpsel g where g.`zbflag` in (1,2) ORDER BY g.`gpflag` DESC ) t;		      
     ...
     ELSEIF (in_mode= 2 or in_mode =3) THEN
           SELECT GROUP_CONCAT(DISTINCT t.`code`) INTO gpcodes  
	      FROM (SELECT CODE FROM gpsel g WHERE  g.`zbflag`=1 ORDER BY g.`gpflag` desc LIMIT 0,21) t;		
	     UPDATE `stratey` SET gpcodes = gpcodes WHERE flag=2;  
...
BEGIN		      
        ...
        SELECT COUNT(DISTINCT p.`code`) into dc_flag FROM POSITION p WHERE p.`inxh` IN (4,22) AND p.`available` > 0;
	 ...
	 /**初始化*/   
        ...
        /**中小创**/
        call cl_getgnztgps(sz_50_zf,cyb_zf,dp_zf,dc_flag,1);
        CALL cl_gethyztgps(sz_50_zf,cyb_zf,dp_zf,dc_flag,2);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `投资计划` AS select `g`.`code` AS `code`,(to_days(`p`.`rq`) - to_days(`get_tran_date`())) AS `days`,`p`.`flag` AS `flag` from (`plan` `p` join `gp` `g`) where ((`p`.`hy` = `g`.`hy`) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) < 20) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) > 4)) union all select `g`.`gpcode` AS `gpcode`,(to_days(`p`.`rq`) - to_days(`get_tran_date`())) AS `days`,`p`.`flag` AS `flag` from (`plan` `p` join `gn2gp` `g`) where ((`p`.`gn` = `g`.`gn`) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) < 20) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) > 4)) union all select `g`.`code` AS `code`,(to_days(`p`.`rq`) - to_days(`get_tran_date`())) AS `days`,`p`.`flag` AS `flag` from (`plan` `p` join `gp` `g`) where ((`p`.`gps` is not null) and (locate(`g`.`name`,`p`.`gps`) > 0) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) < 20) and ((to_days(`p`.`rq`) - to_days(`get_tran_date`())) > 4)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `xsjj` AS select `xsjj$`.`code` AS `code`,`xsjj$`.`name` AS `name`,`xsjj$`.`xsdate` AS `xsdate`,`xsjj$`.`bqjj` AS `bqjj`,`xsjj$`.`dqj` AS `dqj`,`xsjj$`.`zjj` AS `zjj`,`xsjj$`.`zbbl` AS `zbbl` from `xsjj$` where ((`xsjj$`.`zbbl` >= 0.2) and ((to_days(`xsjj$`.`xsdate`) - to_days(now())) > -(45))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ztgn` AS select distinct `gn`.`gn` AS `gn`,`gn`.`gnname` AS `gnname` from (((((`gp` `g` join `gn2gp` `g1`) join `gptoday` `g2`) join `gpzfrec` `g3`) join `gn`) join `gnzfrec`) where ((`g`.`code` = `g1`.`gpcode`) and (`g`.`code` = `g2`.`code`) and (`g`.`code` = `g3`.`code`) and (`gn`.`gn` = `gnzfrec`.`gn`) and (`g1`.`gn` = `gn`.`gn`) and (`g2`.`zf` > 9) and (`g2`.`cj_amt` > 0) and (`g3`.`zf` < 9) and (`g3`.`zf30` < 27)) order by `gnzfrec`.`zf` desc limit 0,1 */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `zthy` AS select `g`.`hy` AS `hy` from ((((`gp` `g` join `hy`) join `gptoday` `g1`) join `gpzfrec` `g2`) join `hyzfrec` `h`) where ((`g`.`code` = `g1`.`code`) and (`g1`.`code` = `g2`.`code`) and (`g`.`hy` = `hy`.`hy`) and (`g`.`hy` = `h`.`hy`) and (`g1`.`cj_amt` > 0) and (`g1`.`zf` > 9) and (`g2`.`zf` < 9) and (`g2`.`zf3` < 27)) order by `h`.`zf` desc limit 0,1 */;
