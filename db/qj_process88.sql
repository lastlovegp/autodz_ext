...
BEGIN
	...
	INSERT INTO qjsel SELECT 0,g.`code`,3,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0 
		...
		AND g2.`rsi_6` < 65			
		AND g.hy NOT IN (SELECT hy FROM 买入行业)
		AND (g.code IN (SELECT DISTINCT CODE FROM 投资计划) OR gs.gn IN ('ld','dpg','rgzn','5g'))	 
      ...
      INSERT INTO qjsel SELECT 0,g2.`code`,5,g4.dqj,g4.zf,CONCAT(DATE_FORMAT(g4.`date`,'%Y-%m-%d'),' ',g4.`trantime`),'0'
        ...
        /*龙头股*/ 
	INSERT INTO qjsel SELECT 0,g.`code`,6,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0
                FROM gpsel gs,gp g,gptoday g1,gpzfrec g2 WHERE gs.`code`=g.`code` AND g.`code`=g1.`code` AND g.`code`=g2.`code`   
                AND gs.zbflag = 2                   
                AND dp_zf > xz_dp_zf       
		AND g1.kpj > 0 
	...
	/**近期低涨幅行业*/
	 ...
	 /** 概念突破 **/
        INSERT INTO qjsel 
		SELECT DISTINCT 0,g.`code`,8,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),g3.gn 
        ...
        /**行业涨停跟进*/
       INSERT INTO qjsel SELECT 0,g.`code`,9,g2.dqj,g2.zf,CONCAT(DATE_FORMAT(g2.`date`,'%Y-%m-%d'),' ',g2.`trantime`),'0'
	 FROM gp g,gpzfrec g1,gptoday g2 WHERE g.`code`=g1.`code` AND g.`code`=g2.`code`
	 AND g.syl < IF(g.hy = '国防军工', 1000, p_syl)   
	 AND xh_9_cnt < 1	
	 AND g2.`cj_amt` > 0 	  
	...
	WHERE 
	  ...
	  ORDER BY g1.zf DESC LIMIT 0,1;
       ...
       /**行业突破 **/
       INSERT INTO qjsel SELECT 0,g.`code`,11,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),g.hy 
	...
	INSERT INTO qjsel SELECT 0,g.`code`,12,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0 
       ...
       /**概念涨停跟进*/
       INSERT INTO qjsel SELECT 0,g.`code`,14,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),g3.`gn`
	 ...
	 INSERT INTO qjsel SELECT 0,g2.`code`,16,g2.dqj,g2.zf,CONCAT(DATE_FORMAT(g2.`date`,'%Y-%m-%d'),' ',g2.`trantime`),0 
	   ...
	   /*活跃概念*/
         ...
         /*阶段上涨*/ 
        INSERT INTO qjsel SELECT 0,g.`code`,17,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0 
