...
) ENGINE=InnoDB AUTO_INCREMENT=179074 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `gptoday` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_code` (`code`)
) ENGINE=InnoDB AUTO_INCREMENT=3415 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `gpyaohhis` (
  ...
  `zf` decimal(8,2) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=32709 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `gpzfrec` (
  ...
  `dt_bar_num` int(11) DEFAULT NULL,
...
CREATE TABLE `gpzhibiao` (
  ...
  `date` date DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=8229 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
BEGIN                 
         ...
         INSERT INTO gpsel SELECT 0,2,SUBSTR(t.`代码`,3),t.`名称`,1,'no',NOW(),1,-1 FROM `table$` t WHERE SUBSTR(t.`代码`,3) NOT IN (SELECT CODE FROM gpsel);                 	
...
BEGIN        
	...
	FROM POSITION p,gptoday g,gp g1,gpsel gs1 
		WHERE p.`code`=g.`code` AND g.code=g1.code and p.code=gs1.code
		 AND p.`available` > 0
		 AND (p.available - p.`available_today`) > 0				
		 AND g.zf < 0	
		 AND (p.`sumzf` < -10
		      or (dp_zf < -1.5 and cyb_zf < -2 and zxp_zf < -2) 
		      or gs1.gn in (SELECT gs.gn FROM gpsel gs,gptoday gt,gpzfrec gr
			 WHERE  gs.`code` = gt.`code` AND gs.`code`=gr.`code`
			  AND gt.`zrspj` >= gr.`top_2` AND gt.`zf` < -2 AND
			  gs.`code` IN ('601318','000651','600581','601015','603019','603501','000063','002714','002594','600295'))
	             )	 							 	 
...
BEGIN
      ...
      INSERT INTO qjsel SELECT DISTINCT 0,g.`code`,in_xh,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),0
		...
		and g1.`cj_amt` > 0		
                ...
                AND TIME_TO_SEC(g1.`trantime`) < TIME_TO_SEC("14:54:00"); 
...
BEGIN
      ...
      SELECT COUNT(p.code) INTO in_cnt FROM POSITION p WHERE  p.`available` > 0; 
            
      /*强势股抄底2*/
      INSERT INTO qjsel SELECT 0,g.`code`,in_xh,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`),gs.`gn`
		...
		AND g1.kpj > 0 
		AND g1.cj_amt > 0		
		AND g1.dqj > 0 and g1.dqj < 50	
		and g1.`zrspj` > 0					
		AND g1.zf < 2 and g1.zf > -3	
		and g1.dqj < g1.`jrzgj`
		and g2.zf < 1
		and g2.zf > -4	
		and (g2.`dt_bar_num` = 0 or g2.`dt_bar_num` > 5)
		and g2.`zf3` < 0																						
	        AND g.hy NOT IN (SELECT hy FROM 买入行业)
	        and gs.`gn` not in (select gpsel.`gn` from position,gpsel where POSITION.available > 0 and POSITION.`code`=gpsel.`code`)	
		AND g.code NOT IN (SELECT p.code FROM `position` p WHERE p.available > 0) 	
		and if(cyb_zf < 0.35 and zxp_zf < 0.25 and dp_zf < 0.15,gs.gpflag>=9,true)
		AND IF(cyb_zf > zxp_zf + dp_zf,gs.gpflag>=9,gs.gpflag<=9)				
		and g1.`zrspj`< g2.`jx_5`*1.002
		AND if(gs.gpflag<9,g1.zrspj < g2.`jx_10`*1.002,true)
		AND IF(gs.gpflag<8,g1.zrspj < g2.`jx_20`*1.002,true)	
		AND IF(gs.gpflag<7,g1.zrspj < g2.`jx_30`*1.002 AND g2.`jx_30` > 0,true)
		AND IF(gs.gpflag<1,g1.zrspj < g2.`jx_60` and g2.`jx_60` > 0,TRUE)		
		AND IF(g1.zf < 1,TIME_TO_SEC(g1.`trantime`) > TIME_TO_SEC("14:40:00"),TRUE)					 												
...
/* Procedure structure for procedure `cl_headgp` */

/*!50003 DROP PROCEDURE IF EXISTS  `cl_headgp` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_headgp`(in_xh int)
BEGIN
      ...
      /*套利*/ 
      INSERT INTO qjsel SELECT DISTINCT 0,gt.`code`,in_xh,gt.dqj,gt.zf,CONCAT(DATE_FORMAT(gt.`date`,'%Y-%m-%d'),' ',gt.`trantime`),0
         FROM gpsel gs,gptoday gt,gpzfrec gr WHERE gs.`code`=gt.`code` AND gs.`code`=gr.`code`
           AND in_cnt < 5
           AND gs.`gn` <> 'zs' 
	   AND gs.`zbflag` = 5 
	   AND gt.kpj > 0 
	   AND gt.cj_amt > 0		
	   AND gt.dqj > 0 AND gt.dqj < 50
	   AND gt.dqj < gt.`jrzgj`
           AND gr.zf < 1
	   AND gr.zf > -4
	   AND (gr.`dt_bar_num` = 0 OR gr.`dt_bar_num` > 5)
	   AND gr.`zf3` < 0		
	   AND gt.`zrspj` > 0	
	   AND gt.`zrspj`< gr.`jx_5` * 1.002 
	   AND gt.`zrspj` < gr.`jx_10` * 1.002  
	   AND gt.`zrspj` < gr.`jx_20` * 1.002  
	   AND gt.`zrspj` < gr.`jx_30` * 1.002
	   AND IF(gs.gpflag<1,gt.zrspj < gr.`jx_60` AND gr.`jx_60` > 0,TRUE)	
	   AND gs.code NOT IN (SELECT POSITION.`code` FROM POSITION WHERE POSITION.`available` > 0)
	   AND gs.`gn` NOT IN (SELECT gpsel.`gn` FROM POSITION,gpsel WHERE POSITION.available > 0 AND POSITION.`code`=gpsel.`code`)
	   AND gt.`cj_amt` > 0
	   AND gt.`zf` > 0 and gt.zf < 4
	   AND TIME_TO_SEC(gt.`trantime`) > TIME_TO_SEC("14:40:00")
	  ORDER BY gs.gpflag desc LIMIT 0,1;       
...
BEGIN
	...
	WHERE 
	  ...
	  AND g.code NOT IN (SELECT p.code FROM POSITION p WHERE p.`available` > 0)	 
	  AND g.code NOT IN (SELECT CODE FROM `xsjj$`)		  	 	 	  
	  ORDER BY hy.`qz` DESC,g.syl LIMIT 0,1;  
...
/* Procedure structure for procedure `cl_tl` */

/*!50003 DROP PROCEDURE IF EXISTS  `cl_tl` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `cl_tl`(in_xh int,mbzf double,mbcode varchar(10),mbgn varchar(10))
BEGIN
      ...
      /*套利*/ 
      INSERT INTO qjsel SELECT DISTINCT 0,gt.`code`,in_xh,gt.dqj,gt.zf,CONCAT(DATE_FORMAT(gt.`date`,'%Y-%m-%d'),' ',gt.`trantime`),0
          FROM gpsel gs,gptoday gt,gpzfrec gr 
          WHERE gs.`gn` = mbgn
          AND in_cnt < 5
          AND gs.`gn` NOT IN (SELECT gpsel.`gn` FROM POSITION,gpsel WHERE POSITION.available > 0 AND POSITION.`code`=gpsel.`code`)
	  AND gs.`code`=gt.`code` AND gs.`code`=gr.`code` 
	  AND gt.`dqj` < gr.`jx_5`*1.002 
	  AND gr.`zf3` < 0
	  AND gt.`zf` > -3 
	  AND gt.`zf` < 1
	  and gt.`cj_amt` > 0
	  ...
	  and gt.dqj < 70
	  AND EXISTS( 
		SELECT g.code FROM gpsel g,gptoday g1,gpzfrec g2 WHERE g.`code` = mbcode
		   AND g.`code`=g1.`code` AND g.`code`=g2.`code` AND g1.`dqj` > g2.`jx_5` AND g1.`dqj` > g2.`jx_10`
		   and g1.dqj < g1.`jrzgj`
		   and g1.cj_amt > 0
		   and g1.dqj > g1.`kpj`
		   and g1.dqj > g1.`jj`
		   AND g1.`zf` > mbzf)
	  AND TIME_TO_SEC(gt.`trantime`) < if(gt.zf < 0.5,TIME_TO_SEC("14:40:00"),TIME_TO_SEC("9:40:00")) 
	  ORDER BY gs.gpflag desc LIMIT 0,1;       
...
END IF;
UPDATE gpsel SET gpflag = -1,zbflag=1 WHERE gpsel.`code` IN (SELECT position.`code` FROM POSITION WHERE POSITION.`sumzf` < -5);
delete from gpsel wHERE gpsel.`code` IN (SELECT position.`code` FROM POSITION WHERE POSITION.`sumzf` < -10);
update position set POSITION.`inxh` = 1 where POSITION.`inxh` = 2;
commit;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `gpflagupd` */

/*!50003 DROP PROCEDURE IF EXISTS  `gpflagupd` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `gpflagupd`()
BEGIN
	 UPDATE gpsel SET gpflag =-1,zbflag=1 WHERE gpsel.`code` IN (SELECT xsjj.code FROM xsjj WHERE xsjj.zbbl >=10) AND gpsel.gpflag < 9;
         UPDATE gpsel SET gpflag =-1,zbflag=1 WHERE gpsel.`code` IN (SELECT CODE FROM gptoday WHERE gptoday.`zf` < -9 AND gptoday.`cj_amt` >0);
         UPDATE gpsel SET gpflag = 3,zbflag=5 WHERE gpsel.`code` IN (SELECT CODE FROM gptoday WHERE gptoday.`zf` > 9 AND gptoday.`cj_amt` >0);  
    END */$$
DELIMITER ;
...
BEGIN
	...
	IF (t_num > 0) THEN		            
		...
		DELETE FROM `position_prd`;
		INSERT INTO POSITION SELECT `证券代码`,t_date,NULL,IFNULL(备注,1), `当前拥股`,`当前拥股`,
			`成本价`,`成本价`,`浮动盈亏`,`盈亏比例(%)`,`盈亏比例(%)`,NULL,0,0,'0' FROM `资金股份查询$` WHERE `当前拥股` > 0;
			
		UPDATE POSITION SET inxh=1 WHERE inxh=0;
    ...
    /*放入股票池*/
     if (in_mode =4) then
	     ...
	     SELECT GROUP_CONCAT(DISTINCT t.`code`) INTO gpcodes  
	      FROM (SELECT CODE FROM gpsel LIMIT 0,5) t;		      
     ...
     ELSEIF (in_mode= 2 or in_mode =3) THEN
           SELECT GROUP_CONCAT(DISTINCT t.`code`) INTO gpcodes  
	      FROM (SELECT CODE FROM gpsel WHERE gpsel.`gn` IN ('jr','gt','mt','bmg','ysjs','xny') 
	      ORDER BY gpsel.`gn`,gpsel.`gpflag` DESC LIMIT 0,21) t;		
	     UPDATE `stratey` SET gpcodes = gpcodes WHERE flag=2;  
...
BEGIN		      
	 ...
	 /**初始化*/   
        ...
        /**套利*/    
        call cl_headgp(1);   
        CALL cl_tl(1,1,'601318','jr');     
        CALL cl_tl(1,2,'000651','bmg');  
        CALL cl_tl(1,2,'600581','gt');   
        CALL cl_tl(1,2,'601015','mt'); 
        CALL cl_tl(1,3,'603019','rgzn');  
        CALL cl_tl(1,4,'603501','bdt');  
        CALL cl_tl(1,3,'000063','5g');   
        CALL cl_tl(1,2,'002714','zr');    
        CALL cl_tl(1,3,'002594','xny');       		
	CALL cl_tl(1,3,'600295','ysjs');
...
BEGIN                 
	...
	DECLARE v_in_date datetime;      
		...
		ELSE                                      			
			...
			WHERE CODE=p_code AND CODE NOT IN ('JM','RB');
		END IF;
	...
	ELSEIF (p_side =-1) THEN
	   ...
	   UPDATE POSITION SET outdate=v_trandate
			...
			WHERE CODE=p_code and vwap >0 and code not in ('JM','RB');
