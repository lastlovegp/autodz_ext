...
BEGIN
      ...
      UPDATE `position` SET sumzf=((SELECT dqj FROM gptoday WHERE position.`code`= gptoday.`code`)-position.`vwap`)/position.`vwap`*100
       ...
       /** 突破 **/
       INSERT INTO qjsel SELECT 0,g.`code`,2,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
	 ...
	 /*SZ50抄底*/
	INSERT INTO qjsel SELECT 0,g.`code`,7,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
                FROM gp g,gptoday g1,gpzfrec g2 WHERE g.`code`=g1.`code` AND g.`code`=g2.`code`
                AND risk_mode = 1
		AND g.ltsz > 1000
		AND g.ltsz < 5000
		...
		AND TIME_TO_SEC(g1.`trantime`) > TIME_TO_SEC(IF(g1.zf >0,"09:35:00","14:31:00"));  
         ...
         /*强势股抄底*/
	INSERT INTO qjsel SELECT 0,g.`code`,15,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
                FROM gpsel gs,gp g,gptoday g1,gpzfrec g2 WHERE gs.`code`=g.`code` AND g.`code`=g1.`code` AND g.`code`=g2.`code`               
		AND g1.kpj > 0 
		AND g1.cj_amt > 0
		AND g1.dqj < 50			
		AND g1.`zrspj` < g2.`jx_30`
		AND g2.`zf` < -5
       ...
       /*行业排名2*/
       INSERT INTO qjsel 
		SELECT 0,g.`code`,11,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
        ...
        /*概念排名2*/
       INSERT INTO qjsel 
		SELECT 0,g.`code`,12,g1.dqj,g1.zf,CONCAT(DATE_FORMAT(g1.`date`,'%Y-%m-%d'),' ',g1.`trantime`) 
		 ...
		 AND g2.zf3 < 10	
		 AND g.syl < 50	
		 AND IF((cyb_zf > 0 AND cyb_zf > dp_zf AND cyb_zf > sz_50_zf),TRUE,FALSE)
		 AND g3.gn NOT IN (SELECT DISTINCT g.gn FROM POSITION p ,gn2gp g WHERE p.`code`=g.gpcode GROUP BY g.`gn` HAVING COUNT(g.gn) > 2)
		 AND g1.dqj < g1.jrzgj	 
		 AND g.ltsz < 5000
		 AND TIME_TO_SEC(g1.`trantime`) > TIME_TO_SEC("9:40:00")
		 AND TIME_TO_SEC(g1.`trantime`) < TIME_TO_SEC("14:40:00")
